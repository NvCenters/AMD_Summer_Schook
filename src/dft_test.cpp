...
int main()
{
	...
	FILE * fp = fopen("E:\\Program\\Fpga\\HLS\\out.gold.dat","r");
	...
	for(int i=0; i<1024; i++)
	{
		...
		//printf("number:%3d Out_R:%7f gold_R:%7f Out_I:%7f gold_I:%7f errorI:%7f\n",index,Out_R[i],gold_R,Out_I[i],gold_I,Out_I[i] - gold_I);
		//printf("number:%3d  gold_R:%7f  gold_I:%7f\n",index,gold_R,gold_I);
	}
	fclose(fp);
